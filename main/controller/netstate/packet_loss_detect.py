...
class PacketLossDetect(app_manager.RyuApp):
    ...
    def save_to_mysql(self, dpid1, dpid2, happen_time,packet_loss_ratio):
        """
        将丢包信息写入mysql数据库，写入时将IP地址转化成字符串形式
        """
        cursor = db1.cursor()
        cursor.execute(insert_sql3, (str(self.packet_loss_id),str(dpid1), str(dpid2), happen_time, str(packet_loss_ratio)))
        self.packet_loss_id = self.packet_loss_id + 1
        db1.commit()
        ...
        while True:
            ...
            for dpid in self.datapaths.keys():
                for nb_dpid in self.get_neighbors(dpid):
                    ...
                    hub.sleep(5)
                    drop_num[(dpid,nb_dpid)] = self.num
            for link in drop_num.keys():
                if drop_num[link] <= DETECT_PACKET_NUM:
                    self.drop_num[link] = drop_num[link]
                else:
                    continue
            ...
            for link in self.drop_num.keys():
                ...
                if packet_loss_ratio > PACKET_LOSS_DETECT_RATIO:
                    ...
                    #dpid1, dpid2, happen_time,packet_loss_ratio
                    self.save_to_mysql(dpid1,dpid2, db_create_time,packet_loss_ratio)
