...
class FlowMeasure(app_manager.RyuApp):
    ...
    def is_in_this_switch(self,dp_id,flow):
        for dpid, test_measure_data in self.new_measure_data.items():
            if dpid == dp_id:
                for five_tuple, flow_data in test_measure_data.measure_data.items():
                    if five_tuple == flow:
    ...
    def heavy_change(self,BitChangeThreshold):
        ...
        for dpid, test_measure_data in self.old_measure_data.items():
            for five_tuple, flow_data in test_measure_data.measure_data.items():
        ...
        while True:
            ...
            for dpid in self.datapaths.keys() :
                ...
                if ret == None or ret == 0:
                    ...
                    ddosed[dpid] = True
            self.ddosed = copy.deepcopy(ddosed)
    ...
    def ddos_inspect(self, dp_id):
        ...
        for dpid, test_measure_data in self.new_measure_data.items():
            if(dpid == dp_id):
                for five_tuple, flow_data in test_measure_data.measure_data.items():
                    if self.new_measure_data[dpid].measure_data[five_tuple].num > 0:
                        flow_count += 1
                        current_label = five_tuple[1]  # 对目的IP进行熵计算。
                        if current_label not in label_counts.keys():
                            label_counts[current_label] = 0
                        packetcount = self.new_measure_data[dpid].measure_data[five_tuple].num
    ...
    def pkt_loss_inspect(self, dp_id):
        #输出这个交换机丢包数
        drop_num = {}
        num = 0
        neighbor_id = self.get_neighbors(dp_id)
        for dpid, test_measure_data in self.new_measure_data.items():
            if dpid == dp_id:
                for five_tuple, flow_data in test_measure_data.measure_data.items():
                    #self.logger.info(self.get_first_switch(five_tuple))
                    #self.logger.info(self.get_last_switch(five_tuple))
                    num == flow_data.num
        for dpid, test_measure_data in self.new_measure_data.items():
            if dpid in neighbor_id:
                for five_tuple, flow_data in test_measure_data.measure_data.items():
                    num_deviation = abs(flow_data.num - num)
                    #self.logger.info(num_deviation)
                    if num_deviation > 10000:
                        drop_num[five_tuple] = num_deviation
        return drop_num

    def pkt_loss_test(self, dp_id):
        # print('pkt_loss_test')
        loss = self.pkt_loss_inspect(dp_id)
        for five_tuple in loss.keys():
            if(five_tuple[4] == 6):
                protocol = "TCP"
            else:
                protocol = "UDP"
        ...
        for dpid, test_measure_data in self.new_measure_data.items():
            for five_tuple, flow_data in test_measure_data.measure_data.items():
                ...
                if prev_id not in self.datapaths.keys():
                    break
                if self.is_in_this_switch(prev_id,five_tuple) != True:
                    continue
                ...
                while not stop and prev_id != last_id and hops < 20:  
                    ...
                    for neighbor_id in self.get_neighbors(prev_id):
                        ...
                        if neighbor_id not in self.datapaths.keys():
                            continue
                        for dp_id, test_measure_data1 in self.new_measure_data.items():
