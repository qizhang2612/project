...
show binary logs;
    ...
    def pub_sub_handler(self, cmd, ip):
        ...
        argv = self._cmd_parser(cmd)
        logging.info("process cmd [" + cmd + "] from " + self.source_ip)
    ...
    def _delete_topic(self, argv: list):
        """Cancel the topic
    ...
    def delete_members(self, channel: str, topic_info: dict):
        ...
        for subscriber in subscribers:
            self.sync_info(subscriber, channel, topic_info)
            if self.results:
                return
            unsubscribe_topic(channel, subscriber)

    def sync_info(self, subscriber: str,
        ...
        sk = get_socket(address=IP, port=SYNC_FROM_PORT,
                        connect_type=socket.SOCK_DGRAM,
                        connect_ip=sub_info['ip'],
                        connect_port=SYNC_TO_PORT)
        if not sk:
            self.results = {"status": "failed", "msg": "socket failed"}
            logging.log("socket failed")
            return
        msg = 'sync' + ' ' + topic_info['ip'] + ' ' + topic_info['location'] + ' ' + channel
        sk.sendto(msg.encode('utf-8'), (sub_info['ip'], SYNC_TO_PORT))
