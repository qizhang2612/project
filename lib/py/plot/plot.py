...
import lib.py.utils.stat as mystat


markers = ('o', 'x', '>', '+', '*')
mstyles = []
for marker in markers:
    mstyle = MarkerStyle(marker)
...
class BarPlot(Plot):
    ...
    def insert_yvals(self, yvals, **plot_kwargs):
        ...
        self.n_groups += 1
        if self.n_yvals < 0:
            self.n_yvals = len(yvals)
        if self.n_yvals != len(yvals):
